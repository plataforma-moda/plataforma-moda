  ...
  return (
    <main style={{ fontFamily: 'Arial, sans-serif', maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      ...
      <section style={{ padding: '30px 0', textAlign: 'center' }}>
        <input type="text" placeholder="Buscar fornecedor, segmento ou cidade..." style={{ width: '60%', padding: '14px 20px', fontSize: '16px', border: '2px solid #1E3A5F', borderRadius: '8px', outline: 'none' }} />
        <button style={{ marginLeft: '10px', padding: '14px 28px', backgroundColor: '#1E3A5F', color: 'white', border: 'none', borderRadius: '8px', fontSize: '16px', cursor: 'pointer' }}>Buscar</button>
      </section>
            ...
            <a key={cat.id} href="/fornecedores" style={{ textDecoration: 'none' }}>
              <div style={{ padding: '20px', border: '1px solid #eee', borderRadius: '12px', backgroundColor: '#f9f9f9', cursor: 'pointer', height: '100%' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                  <span style={{ width: '24px', height: '24px', backgroundColor: '#1E3A5F', color: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: '600', flexShrink: 0 }}>{cat.id}</span>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                  {cat.subcategorias.map((sub) => (
                    <span key={sub} style={{ fontSize: '11px', backgroundColor: '#E6F1FB', color: '#0C447C', padding: '2px 8px', borderRadius: '20px' }}>{sub}</span>
                  ))}
                </div>
              </div>
            </a>
      ...
      <section style={{ textAlign: 'center', padding: '40px', backgroundColor: '#1E3A5F', borderRadius: '16px', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <a href="/cadastro" style={{ display: 'inline-block', padding: '14px 28px', backgroundColor: 'white', color: '#1E3A5F', borderRadius: '8px', fontSize: '15px', fontWeight: 500, textDecoration: 'none' }}>Sou fornecedor</a>
          </div>
          <div style={{ textAlign: 'center' }}>
            <p style={{ color: '#aac4e0', fontSize: '13px', marginBottom: '10px' }}>Busca fornecedores para seu negócio?</p>
            <a href="/clientes" style={{ display: 'inline-block', padding: '14px 28px', backgroundColor: 'transparent', color: 'white', border: '2px solid white', borderRadius: '8px', fontSize: '15px', fontWeight: 500, textDecoration: 'none' }}>Sou comprador</a>
          </div>
        </div>
      </section>
