...
export default function Cadastro() {
  ...
  const [form, setForm] = useState({
    ...
    cep: '', endereço: '', numero: '', complemento: '', bairro: '', cidade: '', estado: '',
      ...
      if (res.ok) {
        ...
        if (cep.length === 8) {
          ...
          if (!cepData.erro) {
            setForm(prev => ({
              ...prev,
              endereço: cepData.logradouro || prev.endereço,
            }))
          }
        }
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (cnpjExistente) { setErro('Este CNPJ ja esta cadastrado.'); return }
    setEnviando(true)
    setErro('')
    ...
    const { data, error } = await supabase.from('fornecedores').insert([{
      ...
      email: form.email, cep: form.cep,
      endereco: form.endereço + (form.numero ? ', ' + form.numero : '') + (form.complemento ? ' - ' + form.complemento : ''),
    ...
    <main style={{ fontFamily: 'Arial, sans-serif', backgroundColor: '#F8FAFC', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '40px 20px' }}>
        ...
        <form onSubmit={handleSubmit}>
              ...
              {cnpjExistente && (
                <div style={{ marginTop: '10px', padding: '16px', backgroundColor: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '8px' }}>
                  <p style={{ fontSize: '13px', color: '#DC2626', marginBottom: '10px', fontWeight: 600 }}>
                    Este CNPJ ja esta cadastrado no SNM.
                  </p>
          ...
          <button type="submit" disabled={enviando || cnpjExistente} style={{ width: '100%', padding: '16px', backgroundColor: enviando || cnpjExistente ? '#94A3B8' : '#0B1F3B', color: 'white', border: 'none', borderRadius: '8px', fontSize: '16px', fontWeight: 600, cursor: enviando || cnpjExistente ? 'not-allowed' : 'pointer', marginBottom: '40px' }}>
